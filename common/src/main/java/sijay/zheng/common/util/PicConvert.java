...
public class PicConvert {
    ...
    public static void main(String[] args) {
        String path = "D:\\bbb\\xx";
        String targetPath = "D:\\bbb\\datmm";
                ...
                .forEach(file1 -> {
                    ...
                    try (InputStream reader = new FileInputStream(file1);
                         OutputStream writer = new FileOutputStream(
                                 targetPath + File.separator + file1.getName()
                                         .split("\\.")[0] + (xor[0] != null ? "." + xor[0] : ""))) {
                        ...
                        System.out.println(
                                file1.getName() + "(大小:" + ((double) file1.length() / 1000) + "kb,异或值:" + xor[1] + ")," + "进度：" + integer.get() + "/" + size);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                });
    ...
    private static Object[] getXor(byte[] bytes) {
        ...
        for (Map.Entry<String, String> type : FILE_TYPE_MAP.entrySet()) {
            String[] hex = {String.valueOf(type.getKey()
                    .charAt(0)) + type.getKey()
                    .charAt(1),
                    String.valueOf(type.getKey()
                            .charAt(2)) + type.getKey()
                            .charAt(3),
                    String.valueOf(type.getKey()
                            .charAt(4)) + type.getKey()
                            .charAt(5)};
