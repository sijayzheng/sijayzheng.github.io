  ...
  <el-card class="main-search" shadow="hover">
    <el-form ref="queryFormRef" :model="queryForm" inline>
      ...
      <el-form-item label="登录ip" prop="ip">
        <el-input v-model="queryForm.ip" clearable placeholder="请输入登录ip" />
      </el-form-item>
      <el-form-item label="登录地点" prop="location">
        <el-input v-model="queryForm.location" clearable placeholder="请输入登录地点" />
      </el-form-item>
      <el-form-item label="浏览器类型" prop="browser">
        <el-input v-model="queryForm.browser" clearable placeholder="请输入浏览器类型" />
      </el-form-item>
      <el-form-item label="操作系统" prop="os">
        <el-input v-model="queryForm.os" clearable placeholder="请输入操作系统" />
      </el-form-item>
      <el-form-item label="登录状态" prop="successful">
        <el-radio-group v-model="queryForm.successful">
          <el-radio v-for="item in successfulOptions" :key="item.value" :label="item.label" :value="item.value" />
        </el-radio-group>
      </el-form-item>
...
const queryForm = ref<LogLogin & PageQuery & { any?: any, arr?: any[] }>({
  userId: undefined,
  ip: undefined,
  location: undefined,
  browser: undefined,
...
const dataForm = ref<LogLogin>({
})
const formRules = ref<FormRules<LogLogin>>({
})
const buttonLoading = ref(false)
const tableData = ref<LogLogin[]>([])
...
function listData() {
  ...
  logLoginApi.page(queryForm.value).then((res) => {
...
function handleUpdate(row: LogLogin) {
  resetForm()
  logLoginApi.getById(row.id).then((res) => {
...
function submitForm() {
  formRef.value?.validate((valid) => {
    if (valid) {
      logLoginApi.save(dataForm.value).then((res) => {
...
function handleDelete(id?: number) {
  ...
  modal.confirm(`是否确认删除部门编号为"${ids}"的数据项？`)
    .then(() => {
      logLoginApi.remove(ids).then((res) => {
        ElMessage.success(res.message)
        listData()
      })
    })
...
function handleExport() {
  logLoginApi.exportData(queryForm.value)
}
