...
const queryForm = ref<SystemDict & PageQuery & { any?: any, arr?: any[] }>({
  code: undefined,
  name: undefined,
  pageNum: 1,
  pageSize: 10,
...
const dataForm = ref<SystemDict>({
  code: undefined,
  name: undefined,
})
const formRules = ref<FormRules<SystemDict>>({
  code: [
...
function listData() {
  ...
  systemDictApi.page(queryForm.value).then((res) => {
...
function handleUpdate(row: SystemDict) {
  resetForm()
  systemDictApi.getById(row.id).then((res) => {
...
function submitForm() {
  formRef.value?.validate((valid) => {
    if (valid) {
      systemDictApi.save(dataForm.value).then((res) => {
...
function handleDelete(id?: number) {
  ...
  modal.confirm(`是否确认删除部门编号为"${ids}"的数据项？`)
    .then(() => {
      systemDictApi.remove(ids).then((res) => {
        ElMessage.success(res.message)
        listData()
      })
    })
...
function handleExport() {
  systemDictApi.exportData(queryForm.value)
}
...
function downloadTemplate() {
  systemDictApi.downloadTemplate()
}
