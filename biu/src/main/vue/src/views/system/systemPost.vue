<template>
  <el-card class="main-search" shadow="hover">
    <el-form ref="queryFormRef" :model="queryForm" inline>
      ...
      <el-form-item label="部门id" prop="deptId">
        <el-tree-select
                v-model="queryForm.deptId" :data="deptIdOptions" :filter-node-method=" (v, data) => data.label.includes(v)"
                check-strictly
        />
      </el-form-item>
      <el-form-item>
  ...
  <el-card class="main-display" shadow="never">
    ...
    >
      ...
      <el-table-column label="部门id" min-width="180" prop="deptId" sortable />
...
const queryForm = ref<SystemPost & PageQuery & { any?: any, arr?: any[] }>({
  name: undefined,
  code: undefined,
  deptId: undefined,
  pageNum: 1,
...
const formRules = ref<FormRules<SystemPost>>({
  name: [
  ...
  deptId: [
    { required: true, message: '部门id不可为空', trigger: 'blur' },
  ],
})
const buttonLoading = ref(false)
const tableData = ref<SystemPost[]>([])
...
function listData() {
  ...
  systemPostApi.page(queryForm.value).then((res) => {
...
function handleUpdate(row: SystemPost) {
  resetForm()
  systemPostApi.getById(row.id).then((res) => {
...
function submitForm() {
  formRef.value?.validate((valid) => {
    if (valid) {
      systemPostApi.save(dataForm.value).then((res) => {
...
function handleDelete(id?: number) {
  ...
  modal.confirm(`是否确认删除部门编号为"${ids}"的数据项？`)
    .then(() => {
      systemPostApi.remove(ids).then((res) => {
        ElMessage.success(res.message)
        listData()
      })
    })
...
function handleExport() {
  systemPostApi.exportData(queryForm.value)
}
...
function downloadTemplate() {
  systemPostApi.downloadTemplate()
}
