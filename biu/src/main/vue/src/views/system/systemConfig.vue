<template>
  ...
  <el-card class="main-display" shadow="never">
    ...
    >
      ...
      <el-table-column label="值" min-width="180" prop="value" sortable />
      <el-table-column label="内置" min-width="180" prop="internal" sortable />
  ...
  <el-dialog v-model="dialog.visible" :title="dialog.title" append-to-body width="1000px">
    <el-form ref="formRef" :model="dataForm" :rules="formRules" label-width="120px" status-icon>
      ...
      <el-form-item label="值" prop="value">
        <el-input v-model="dataForm.value" clearable placeholder="请输入值" />
      </el-form-item>
      <el-form-item label="内置" prop="internal">
        <el-radio-group v-model="dataForm.internal">
          <el-radio v-for="item in internalOptions" :key="item.value" :label="item.label" :value="item.value" />
        </el-radio-group>
      </el-form-item>
    </el-form>
    <template #footer>
...
const queryForm = ref<SystemConfig & PageQuery & { any?: any, arr?: any[] }>({
  name: undefined,
  code: undefined,
  pageNum: 1,
  pageSize: 10,
...
const dataForm = ref<SystemConfig>({
  name: undefined,
  code: undefined,
  value: undefined,
  internal: undefined,
})
const formRules = ref<FormRules<SystemConfig>>({
  name: [
    { required: true, message: '名称不可为空', trigger: 'blur' },
    { max: 100, message: '名称最大长度为100', trigger: 'blur' },
  ],
  code: [
    { required: true, message: '编码不可为空', trigger: 'blur' },
    { max: 100, message: '编码最大长度为100', trigger: 'blur' },
  ],
  value: [
    { required: true, message: '值不可为空', trigger: 'blur' },
    { max: 500, message: '值最大长度为500', trigger: 'blur' },
  ],
})
...
function listData() {
  ...
  systemConfigApi.page(queryForm.value).then((res) => {
...
function handleUpdate(row: SystemConfig) {
  resetForm()
  systemConfigApi.getById(row.id).then((res) => {
...
function submitForm() {
  formRef.value?.validate((valid) => {
    if (valid) {
      systemConfigApi.save(dataForm.value).then((res) => {
...
function handleDelete(id?: number) {
  ...
  modal.confirm(`是否确认删除部门编号为"${ids}"的数据项？`)
    .then(() => {
      systemConfigApi.remove(ids).then((res) => {
        ElMessage.success(res.message)
        listData()
      })
    })
...
function handleExport() {
  systemConfigApi.exportData(queryForm.value)
}
...
function downloadTemplate() {
  systemConfigApi.downloadTemplate()
}
