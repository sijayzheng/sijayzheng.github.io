  ...
  <el-dialog v-model="dialog.visible" :title="dialog.title" append-to-body width="1000px">
    <el-form ref="formRef" :model="dataForm" :rules="formRules" label-width="120px" status-icon>
      ...
      <el-form-item label="角色" prop="roles">
        <el-select v-model="dataForm.roles" clearable multiple placeholder="请选择角色">
...
const queryForm = ref<SystemModule & PageQuery & { any?: any, arr?: any[] }>({
  code: undefined,
  name: undefined,
  pageNum: 1,
  pageSize: 10,
...
const dataForm = ref<SystemModule>({
  code: undefined,
  name: undefined,
  roles: undefined,
  menus: undefined,
...
const formRules = ref<FormRules<SystemModule>>({
  code: [
...
const tableData = ref<SystemModule[]>([])
...
function listData() {
  ...
  systemModuleApi.page(queryForm.value).then((res) => {
...
function handleUpdate(row: SystemModule) {
  resetForm()
  systemModuleApi.getById(row.id).then((res) => {
...
function submitForm() {
  formRef.value?.validate((valid) => {
    if (valid) {
      systemModuleApi.save(dataForm.value).then((res) => {
...
function handleDelete(id?: number) {
  ...
  modal.confirm(`是否确认删除部门编号为"${ids}"的数据项？`)
    .then(() => {
      systemModuleApi.remove(ids).then((res) => {
        ElMessage.success(res.message)
        listData()
      })
    })
...
function handleExport() {
  systemModuleApi.exportData(queryForm.value)
}
...
function downloadTemplate() {
  systemModuleApi.downloadTemplate()
}
