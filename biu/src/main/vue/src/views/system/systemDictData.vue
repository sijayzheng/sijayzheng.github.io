<template>
  <el-card class="main-search" shadow="hover">
    <el-form ref="queryFormRef" :model="queryForm" inline>
      <el-form-item label="字典" prop="dictId">
        <el-select v-model="queryForm.dictId" clearable placeholder="请选择字典">
          <el-option v-for="item in dictIdOptions" :key="item.value" :label="item.label" :value="item.value" />
        </el-select>
      </el-form-item>
      <el-form-item label="标签" prop="label">
        <el-input v-model="queryForm.label" clearable placeholder="请输入标签" />
      </el-form-item>
      <el-form-item>
  ...
  <el-card class="main-display" shadow="never">
    ...
    >
      ...
      <el-table-column label="字典" min-width="180" prop="dictId" sortable />
      <el-table-column label="标签" min-width="180" prop="label" sortable />
      <el-table-column label="键值" min-width="180" prop="value" sortable />
      <el-table-column label="排序" min-width="180" prop="sort" sortable />
      <el-table-column label="样式属性" min-width="180" prop="className" sortable />
      <el-table-column label="表格回显样式" min-width="180" prop="showType" sortable />
      <el-table-column label="默认值" min-width="180" prop="defaultValue" sortable />
  ...
  <el-dialog v-model="dialog.visible" :title="dialog.title" append-to-body width="1000px">
    <el-form ref="formRef" :model="dataForm" :rules="formRules" label-width="120px" status-icon>
      <el-form-item label="字典" prop="dictId">
        <el-select v-model="dataForm.dictId" clearable multiple placeholder="请选择字典">
          <el-option v-for="item in dictIdOptions" :key="item.value" :label="item.label" :value="item.value" />
        </el-select>
      </el-form-item>
      <el-form-item label="标签" prop="label">
        <el-input v-model="dataForm.label" clearable placeholder="请输入标签" />
      </el-form-item>
      <el-form-item label="键值" prop="value">
        <el-input v-model="dataForm.value" clearable placeholder="请输入键值" />
      </el-form-item>
      <el-form-item label="排序" prop="sort">
        <el-input-number v-model="dataForm.sort" :precision="0" :step="1" />
      </el-form-item>
      <el-form-item label="样式属性" prop="className">
        <el-input v-model="dataForm.className" clearable placeholder="请输入样式属性" />
      </el-form-item>
      <el-form-item label="表格回显样式" prop="showType">
        <el-select v-model="dataForm.showType" clearable multiple placeholder="请选择表格回显样式">
          <el-option v-for="item in showTypeOptions" :key="item.value" :label="item.label" :value="item.value" />
        </el-select>
      </el-form-item>
      <el-form-item label="默认值" prop="defaultValue">
        <el-radio-group v-model="dataForm.defaultValue">
          <el-radio v-for="item in defaultValueOptions" :key="item.value" :label="item.label" :value="item.value" />
        </el-radio-group>
      </el-form-item>
    </el-form>
    <template #footer>
...
const queryForm = ref<SystemDictData & PageQuery & { any?: any, arr?: any[] }>({
  dictId: undefined,
  label: undefined,
  pageNum: 1,
  pageSize: 10,
...
const dataForm = ref<SystemDictData>({
  dictId: undefined,
  label: undefined,
  value: undefined,
  sort: undefined,
...
const formRules = ref<FormRules<SystemDictData>>({
  dictId: [
    { required: true, message: '字典不可为空', trigger: 'blur' },
  ],
  label: [
    { required: true, message: '标签不可为空', trigger: 'blur' },
    { max: 100, message: '标签最大长度为100', trigger: 'blur' },
  ],
  value: [
    { required: true, message: '键值不可为空', trigger: 'blur' },
    { max: 100, message: '键值最大长度为100', trigger: 'blur' },
  ],
  className: [
    { max: 100, message: '样式属性最大长度为100', trigger: 'blur' },
  ],
  showType: [
    { max: 7, message: '表格回显样式最大长度为7', trigger: 'blur' },
  ],
})
const buttonLoading = ref(false)
const tableData = ref<SystemDictData[]>([])
...
const dictIdOptions = ref<Option[]>([])
const showTypeOptions = ref<Option[]>([])
const defaultValueOptions = ref<Option[]>([])
...
function listData() {
  ...
  systemDictDataApi.page(queryForm.value).then((res) => {
...
function handleUpdate(row: SystemDictData) {
  resetForm()
  systemDictDataApi.getById(row.id).then((res) => {
...
function submitForm() {
  formRef.value?.validate((valid) => {
    if (valid) {
      systemDictDataApi.save(dataForm.value).then((res) => {
...
function handleDelete(id?: number) {
  ...
  modal.confirm(`是否确认删除部门编号为"${ids}"的数据项？`)
    .then(() => {
      systemDictDataApi.remove(ids).then((res) => {
        ElMessage.success(res.message)
        listData()
      })
    })
...
function handleExport() {
  systemDictDataApi.exportData(queryForm.value)
}
...
function downloadTemplate() {
  systemDictDataApi.downloadTemplate()
}
