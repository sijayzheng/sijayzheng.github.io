<template>
  ...
  <el-card class="main-display" shadow="never">
    ...
    >
      ...
      <el-table-column label="部门名称" min-width="180" prop="name" sortable />
      <el-table-column label="父部门id" min-width="180" prop="parentId" sortable />
      <el-table-column label="排序" min-width="180" prop="sort" sortable />
      <el-table-column label="负责人" min-width="180" prop="leader" sortable />
      <el-table-column label="联系电话" min-width="180" prop="phone" sortable />
  ...
  <el-dialog v-model="dialog.visible" :title="dialog.title" append-to-body width="1000px">
    <el-form ref="formRef" :model="dataForm" :rules="formRules" label-width="120px" status-icon>
      ...
      <el-form-item label="负责人" prop="leader">
        <el-select v-model="dataForm.leader" clearable placeholder="请选择负责人">
          <el-option v-for="item in leaderOptions" :key="item.value" :label="item.label" :value="item.value" />
        </el-select>
      </el-form-item>
      <el-form-item label="联系电话" prop="phone">
        <el-input v-model="dataForm.phone" clearable placeholder="请输入联系电话" />
      </el-form-item>
...
const queryForm = ref<SystemDept & PageQuery & { any?: any, arr?: any[] }>({
  name: undefined,
  parentId: undefined,
  pageNum: 1,
  pageSize: 10,
...
const formRules = ref<FormRules<SystemDept>>({
  name: [
    { required: true, message: '部门名称不可为空', trigger: 'blur' },
    { max: 50, message: '部门名称最大长度为50', trigger: 'blur' },
  ],
  phone: [
    { max: 11, message: '联系电话最大长度为11', trigger: 'blur' },
  ],
  email: [
...
const tableData = ref<SystemDept[]>([])
...
function listData() {
  ...
  systemDeptApi.page(queryForm.value).then((res) => {
...
function handleUpdate(row: SystemDept) {
  resetForm()
  systemDeptApi.getById(row.id).then((res) => {
...
function submitForm() {
  formRef.value?.validate((valid) => {
    if (valid) {
      systemDeptApi.save(dataForm.value).then((res) => {
...
function handleDelete(id?: number) {
  ...
  modal.confirm(`是否确认删除部门编号为"${ids}"的数据项？`)
    .then(() => {
      systemDeptApi.remove(ids).then((res) => {
        ElMessage.success(res.message)
        listData()
      })
    })
...
function handleExport() {
  systemDeptApi.exportData(queryForm.value)
}
...
function downloadTemplate() {
  systemDeptApi.downloadTemplate()
}
