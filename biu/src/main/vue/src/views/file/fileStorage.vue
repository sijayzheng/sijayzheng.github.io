  ...
  <el-card class="main-search" shadow="hover">
    <el-form ref="queryFormRef" :model="queryForm" inline>
      <el-form-item label="原名" prop="originalName">
        <el-input v-model="queryForm.originalName" clearable placeholder="请输入原名" />
      </el-form-item>
      <el-form-item>
...
const queryForm = ref<FileStorage & PageQuery & { any?: any, arr?: any[] }>({
  originalName: undefined,
  pageNum: 1,
  pageSize: 10,
})
const formRef = ref<FormInstance>()
const dataForm = ref<FileStorage>({
})
const formRules = ref<FormRules<FileStorage>>({
})
const buttonLoading = ref(false)
const tableData = ref<FileStorage[]>([])
...
function listData() {
  ...
  fileStorageApi.page(queryForm.value).then((res) => {
...
function handleUpdate(row: FileStorage) {
  resetForm()
  fileStorageApi.getById(row.id).then((res) => {
...
function submitForm() {
  formRef.value?.validate((valid) => {
    if (valid) {
      fileStorageApi.save(dataForm.value).then((res) => {
...
function handleDelete(id?: number) {
  ...
  modal.confirm(`是否确认删除部门编号为"${ids}"的数据项？`)
    .then(() => {
      fileStorageApi.remove(ids).then((res) => {
        ElMessage.success(res.message)
        listData()
      })
    })
...
function handleExport() {
  fileStorageApi.exportData(queryForm.value)
}
...
function downloadTemplate() {
  fileStorageApi.downloadTemplate()
}
