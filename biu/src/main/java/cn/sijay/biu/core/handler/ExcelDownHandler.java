...
import cn.sijay.biu.core.annotation.ExcelDictFormat;
import cn.sijay.biu.core.annotation.ExcelEnumFormat;
import cn.sijay.biu.core.entity.DropDownOptions;
import cn.sijay.biu.core.util.CollectionUtil;
import cn.sijay.biu.core.util.EnumUtil;
...
//        this.dictService = SpringUtil.getBean(SystemDictService.class);
    }
...
//                    options = new ArrayList<>(values);
        ...
        if (CollectionUtil.isEmpty(dropDownOptions)) {
            return;
        }
    ...
    private void dropDownLinkedOptions(DataValidationHelper helper, Workbook workbook, Sheet sheet, DropDownOptions options) {
        ...
        for (int columIndex = 0; columIndex < firstOptions.size(); columIndex++) {
            ...
            if (CollectionUtil.isEmpty(secondOptions)) {
    ...
    private String getExcelColumnName(int columnIndex) {
        ...
        String columnPrefix = columnCircleCount == 0 ? StringUtil.EMPTY : EXCEL_COLUMN_NAME.substring(columnCircleCount - 1, 1);
        // 从26一循环内取对应的栏位名
        String columnNext = EXCEL_COLUMN_NAME.substring(thisCircleColumnIndex, 1);
        // 将二者拼接即为最终的栏位名
