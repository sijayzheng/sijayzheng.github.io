...
<img src="/img/3aac48106dfd000f3aec4349289cfbbf.png" alt="Vue 实例生命周期" style="zoom:50%;" />
...
- **用法**：
  ...
  从 2.6 起，`v-for` 也可以在实现了可迭代协议(https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Iteration_protocols#可迭代协议)的值上使用，包括原生的 `Map` 和 `Set`。不过应该注意的是 Vue 2.x 目前并不支持可响应的 `Map` 和 `Set` 值，所以无法自动探测变更。
