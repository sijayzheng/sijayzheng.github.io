...
1. 安装 jdk，配置环境变量
2. 解压 maven，配置环境变量：M2_HOME 和 MAVEN_HOME，path 中添加%M2_HOME%\bin
3. 在 cmd 中通过 mvn -version 验证

## 配置

- 本地资源库

修改{M2_HOME}/conf/setting.xml 文件中的 localRepository
...
添加 JBoss 远程仓库的详细信息在 “pom.xml” 文件中。
...
### maven 因为约定大于配置，所以可能会存在配置文件无法导出或生效的问题
