...
public class PictureUtil {
        ...
        Arrays.stream(file)
              ...
              .forEach(file1 -> {
                  if (file1.isDirectory()) {
                      String[] newTargetPath = file1.getPath()
                                                    .split("[/\\\\]");
                  ...
                  try (InputStream reader = new FileInputStream(file1);
                       OutputStream writer = new FileOutputStream(targetPath + File.separator +
                           file1.getName().split("\\.")[0] + (xor[0] != null ? "." + xor[0] : ""))) {
                      ...
                      System.out.println(file1.getName() + "(大小:" + ((double) file1.length() / 1000) + "kb,异或值:" + xor[1] + ")," + "进度：" + integer.get() + "/" + size);
                  } catch (Exception e) {
                      e.printStackTrace();
                  }
              });
    ...
    private static Object[] getXor(File file) {
        ...
        if (file != null) {
            ...
            try (InputStream reader = new FileInputStream(file)) {
                ...
                throw new BaseException(ExceptionConstant.XOR_CALC_ERROR);
            }
            xor = getXor(bytes);
        }
        return xor;
    ...
    private static Object[] getXor(byte[] bytes) {
        ...
        for (Map.Entry<String, String> type : FILE_TYPE_MAP.entrySet()) {
            String[] hex = {String.valueOf(type.getKey().charAt(0)) + type.getKey().charAt(1),
