...
import cn.sijay.suap.log.mapper.LogLoginMapper;
...
/**
 * <p>
 * <em>LogLoginService 登录日志服务层实现类</em>
 * </p>
 *
 * @author Sijay
...
public class LogLoginServiceImpl extends ServiceImpl<LogLoginMapper, LogLogin> implements ILogLoginService {
    ...
    public Page<LogLogin> page(LogLogin entity, PageQuery pageQuery) {
        return page(pageQuery.build(), buildQueryWrapper(entity, LogLogin.class));
    }
    ...
    public boolean update(LogLogin entity) {
        if (checkUnique(entity) && exists(entity.getId())) {
            LogLogin logLogin = getById(entity.getId());
            BeanUtil.copyEntityProperties(entity, logLogin);
            return updateById(logLogin);
        }
        return false;
    }
    ...
    public String importData(MultipartFile file) {
        int successCount = 0;
        int failCount = 0;
        for (LogLogin entity : ExcelUtil.read(file, LogLogin.class)) {
            if (add(entity)) {
                successCount++;
            } else {
                failCount++;
