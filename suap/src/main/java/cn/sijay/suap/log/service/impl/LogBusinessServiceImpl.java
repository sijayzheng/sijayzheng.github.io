package cn.sijay.suap.log.service.impl;
...
import cn.sijay.suap.log.mapper.LogBusinessMapper;
...
/**
 * <p>
 * <em>LogBusinessService 业务日志服务层实现类</em>
 * </p>
 *
 * @author Sijay
...
public class LogBusinessServiceImpl extends ServiceImpl<LogBusinessMapper, LogBusiness> implements ILogBusinessService {
    ...
    private final LogBusinessMapper logBusinessMapper;
     ...
     */
    @Override
    public Page<LogBusiness> page(LogBusiness entity, PageQuery pageQuery) {
        return page(pageQuery.build(), buildQueryWrapper(entity, LogBusiness.class));
    }
    ...
    public boolean add(LogBusiness entity) {
    ...
    @Override
    @Transactional(rollbackFor = {Exception.class})
    public boolean update(LogBusiness entity) {
        if (checkUnique(entity) && exists(entity.getId())) {
            LogBusiness logBusiness = getById(entity.getId());
            BeanUtil.copyEntityProperties(entity, logBusiness);
            return updateById(logBusiness);
        }
        return false;
    }
    ...
    public String importData(MultipartFile file) {
        int successCount = 0;
        int failCount = 0;
        for (LogBusiness entity : ExcelUtil.read(file, LogBusiness.class)) {
            if (add(entity)) {
                successCount++;
            } else {
                failCount++;
