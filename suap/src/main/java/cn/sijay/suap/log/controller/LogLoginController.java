...
import cn.sijay.suap.log.service.ILogLoginService;
...
/**
 * <p>
 * <em>LogLoginController 登录日志控制层</em>
 * </p>
 *
 * @author Sijay
 * @since 2024/02/04 15:17
 */
@Tag(name = "登录日志", description = "/logLogin")
...
public class LogLoginController extends BaseController {
    ...
    @ApiResponse(responseCode = "200", description = "登录日志详细信息")
    @GetMapping("getById")
     ...
     * 查询所有记录
     *
     * @return 登录日志信息列表，如果没有记录，则返回空列表。
     */
    @Log(value = "查询所有记录", operateType = OperateType.SELECT)
    @Operation(summary = "查询所有记录")
    @ApiResponse(responseCode = "200", description = "登录日志信息列表，如果没有记录，则返回空列表。")
    @GetMapping("listAll")
    public Result<List<LogLogin>> listAll() {
        return success(logLoginService.list());
    }

    /**
     * 分页查询
     *
     * @param logLogin  查询条件，可以为空。如果为空，则查询所有记录。
    ...
    @ApiResponse(responseCode = "200", description = "登录日志信息列表，如果没有记录，则返回空列表。")
    @GetMapping("page")
    public PageResult<LogLogin> page(LogLogin logLogin, PageQuery pageQuery) {
        return toPageResult(logLoginService.page(logLogin, pageQuery));
    }

    /**
     * 添加
     *
     * @param logLogin 添加的登录日志信息，不能为空。如果为空，则添加失败。
     * @return 是否添加成功
     */
    @Log(value = "添加", operateType = OperateType.CREATE)
    @Operation(summary = "添加")
    @Parameter(name = "logLogin", description = "添加的登录日志信息，不能为空。如果为空，则添加失败。")
    @ApiResponse(responseCode = "200", description = "是否添加成功")
    @PostMapping("add")
    public Result<Boolean> add(@RequestBody LogLogin logLogin) {
        return toBoolean(logLoginService.add(logLogin), OperateType.CREATE);
    }

    /**
     * 修改
     *
     * @param logLogin 修改的登录日志信息，不能为空。如果为空，则添加失败。
     * @return 是否修改成功
     */
    @Log(value = "修改", operateType = OperateType.MODIFY)
    @Operation(summary = "修改")
    @Parameter(name = "logLogin", description = "修改的登录日志信息，不能为空。如果为空，则添加失败。")
    @ApiResponse(responseCode = "200", description = "是否修改成功")
    @PostMapping("update")
    public Result<Boolean> update(@RequestBody LogLogin logLogin) {
        return toBoolean(logLoginService.update(logLogin), OperateType.MODIFY);
    }

    /**
     * 删除
     *
     * @param ids 要删除的登录日志id，不能为空。如果为空，则删除失败。
     * @return 是否删除成功
     */
    @Log(value = "删除", operateType = OperateType.REMOVE)
    @Operation(summary = "删除")
    @Parameter(name = "ids", description = "要删除的登录日志id，不能为空。如果为空，则删除失败。")
    @ApiResponse(responseCode = "200", description = "是否删除成功")
    @PostMapping("remove")
    public Result<Boolean> remove(@RequestBody Ids<Long> ids) {
        return toBoolean(logLoginService.remove(ids), OperateType.REMOVE);
    }
    ...
    public Result<Void> importData(@RequestPart("file") MultipartFile file) {
        return success(logLoginService.importData(file));
    }
    ...
    public void export(HttpServletResponse response) {
        ExcelUtil.export(logLoginService.list(), "登录日志", LogLogin.class, response);
    }

    /**
     * 下载登录日志_数据模板
     */
    @Log(value = "下载登录日志_数据模板", operateType = OperateType.DOWNLOAD)
    @Operation(summary = "下载登录日志_数据模板")
    @ApiResponse(responseCode = "200", description = "是否下载成功")
    @PostMapping("downloadTemplate")
    public void downloadTemplate(HttpServletResponse response) {
        ExcelUtil.exportTemplate("登录日志_数据模板", LogLogin.class, response);
    }
