...
/**
 * <p>
 * <em>SysUserController 登录用户控制层</em>
 * </p>
 *
 * @author Sijay
...
public class SysUserController extends BaseController {
    ...
    @GetMapping("getById")
    public Result<SysUser> getById(@RequestParam("id") Long id) {
     ...
     * 分页查询
     *
     * @param sysUser   查询条件，可以为空。如果为空，则查询所有记录。
     ...
     * 添加
     *
     * @param sysUser 添加的登录用户信息，不能为空。如果为空，则添加失败。
     * @return 是否添加成功
     */
    ...
    public Result<Boolean> add(@RequestBody SysUser sysUser) {
        return toBoolean(sysUserService.add(sysUser), OperateType.CREATE);
    }

    /**
     * 修改
     *
     * @param sysUser 修改的登录用户信息，不能为空。如果为空，则添加失败。
     * @return 是否修改成功
     */
    ...
    public Result<Boolean> update(@RequestBody SysUser sysUser) {
        return toBoolean(sysUserService.update(sysUser), OperateType.MODIFY);
    }

    /**
     * 删除
     *
     * @param ids 要删除的登录用户id，不能为空。如果为空，则删除失败。
     * @return 是否删除成功
     */
    @Log(value = "删除", operateType = OperateType.REMOVE)
    @Operation(summary = "删除")
    @Parameter(name = "ids", description = "要删除的登录用户id，不能为空。如果为空，则删除失败。")
    @ApiResponse(responseCode = "200", description = "是否删除成功")
    @PostMapping("remove")
    public Result<Boolean> remove(@RequestBody Ids<Long> ids) {
        return toBoolean(sysUserService.remove(ids), OperateType.REMOVE);
    }
    ...
    public void export(HttpServletResponse response) {
        ExcelUtil.export(sysUserService.list(), "登录用户", SysUser.class, response);
    }

    /**
     * 下载登录用户_数据模板
     */
    @Log(value = "下载登录用户_数据模板", operateType = OperateType.DOWNLOAD)
    @Operation(summary = "下载登录用户_数据模板")
    @ApiResponse(responseCode = "200", description = "是否下载成功")
    @PostMapping("downloadTemplate")
    public void downloadTemplate(HttpServletResponse response) {
        ExcelUtil.exportTemplate("登录用户_数据模板", SysUser.class, response);
    }
