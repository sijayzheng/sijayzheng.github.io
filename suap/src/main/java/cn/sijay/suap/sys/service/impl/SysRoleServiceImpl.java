...
import cn.sijay.suap.sys.mapper.SysRoleMapper;
...
/**
 * <p>
 * <em>SysRoleService 角色信息服务层实现类</em>
 * </p>
 *
 * @author Sijay
...
public class SysRoleServiceImpl extends ServiceImpl<SysRoleMapper, SysRole> implements ISysRoleService {
     ...
     */
    @Override
    public Page<SysRole> page(SysRole entity, PageQuery pageQuery) {
        return page(pageQuery.build(), buildQueryWrapper(entity, SysRole.class));
    }
    ...
    public boolean update(SysRole entity) {
        if (checkUnique(entity) && exists(entity.getId())) {
            SysRole sysRole = getById(entity.getId());
            BeanUtil.copyEntityProperties(entity, sysRole);
    ...
    public String importData(MultipartFile file) {
        int successCount = 0;
        int failCount = 0;
        for (SysRole entity : ExcelUtil.read(file, SysRole.class)) {
            if (add(entity)) {
                successCount++;
            } else {
                failCount++;
