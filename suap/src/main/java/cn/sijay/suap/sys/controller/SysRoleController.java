...
/**
 * <p>
 * <em>SysRoleController 角色信息控制层</em>
 * </p>
 *
 * @author Sijay
...
public class SysRoleController extends BaseController {
    ...
    @GetMapping("getById")
    public Result<SysRole> getById(@RequestParam("id") Long id) {
     ...
     * 分页查询
     *
     * @param sysRole   查询条件，可以为空。如果为空，则查询所有记录。
     ...
     * 添加
     *
     * @param sysRole 添加的角色信息信息，不能为空。如果为空，则添加失败。
     * @return 是否添加成功
     */
    ...
    public Result<Boolean> add(@RequestBody SysRole sysRole) {
        return toBoolean(sysRoleService.add(sysRole), OperateType.CREATE);
    }

    /**
     * 修改
     *
     * @param sysRole 修改的角色信息信息，不能为空。如果为空，则添加失败。
     * @return 是否修改成功
     */
    ...
    public Result<Boolean> update(@RequestBody SysRole sysRole) {
        return toBoolean(sysRoleService.update(sysRole), OperateType.MODIFY);
    }

    /**
     * 删除
     *
     * @param ids 要删除的角色信息id，不能为空。如果为空，则删除失败。
     * @return 是否删除成功
     */
    @Log(value = "删除", operateType = OperateType.REMOVE)
    @Operation(summary = "删除")
    @Parameter(name = "ids", description = "要删除的角色信息id，不能为空。如果为空，则删除失败。")
    @ApiResponse(responseCode = "200", description = "是否删除成功")
    @PostMapping("remove")
    public Result<Boolean> remove(@RequestBody Ids<Long> ids) {
        return toBoolean(sysRoleService.remove(ids), OperateType.REMOVE);
    }
    ...
    public void export(HttpServletResponse response) {
        ExcelUtil.export(sysRoleService.list(), "角色信息", SysRole.class, response);
    }

    /**
     * 下载角色信息_数据模板
     */
    @Log(value = "下载角色信息_数据模板", operateType = OperateType.DOWNLOAD)
    @Operation(summary = "下载角色信息_数据模板")
    @ApiResponse(responseCode = "200", description = "是否下载成功")
    @PostMapping("downloadTemplate")
    public void downloadTemplate(HttpServletResponse response) {
        ExcelUtil.exportTemplate("角色信息_数据模板", SysRole.class, response);
    }
