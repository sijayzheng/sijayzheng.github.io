...
import cn.sijay.suap.sys.mapper.SysUserMapper;
...
/**
 * <p>
 * <em>SysUserService 登录用户服务层实现类</em>
 * </p>
 *
 * @author Sijay
...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements ISysUserService {
     ...
     */
    @Override
    public Page<SysUser> page(SysUser entity, PageQuery pageQuery) {
        return page(pageQuery.build(), buildQueryWrapper(entity, SysUser.class));
    }
    ...
    public boolean update(SysUser entity) {
        if (checkUnique(entity) && exists(entity.getId())) {
            ...
            BeanUtil.copyEntityProperties(entity, sysUser);
    ...
    public String importData(MultipartFile file) {
        int successCount = 0;
        int failCount = 0;
        for (SysUser entity : ExcelUtil.read(file, SysUser.class)) {
            if (add(entity)) {
                successCount++;
            } else {
                failCount++;
    ...
    public List<Option<Long>> getOptionData() {
        return list()
                .stream().map(item -> new Option<>(item.getUsername(), item.getId()))
