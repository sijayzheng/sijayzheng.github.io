...
import cn.sijay.suap.sys.mapper.SysConfigMapper;
...
/**
 * <p>
 * <em>SysConfigService 系统配置服务层实现类</em>
 * </p>
 *
 * @author Sijay
...
public class SysConfigServiceImpl extends ServiceImpl<SysConfigMapper, SysConfig> implements ISysConfigService {
     ...
     */
    @Override
    public Page<SysConfig> page(SysConfig entity, PageQuery pageQuery) {
        return page(pageQuery.build(), buildQueryWrapper(entity, SysConfig.class));
    }
    ...
    public boolean update(SysConfig entity) {
        if (checkUnique(entity) && exists(entity.getId())) {
            SysConfig sysConfig = getById(entity.getId());
            BeanUtil.copyEntityProperties(entity, sysConfig);
    ...
    public String importData(MultipartFile file) {
        int successCount = 0;
        int failCount = 0;
        for (SysConfig entity : ExcelUtil.read(file, SysConfig.class)) {
            if (add(entity)) {
                successCount++;
            } else {
                failCount++;
