...
import cn.sijay.suap.sys.mapper.SysNoticeMapper;
...
/**
 * <p>
 * <em>SysNoticeService 通知公告服务层实现类</em>
 * </p>
 *
 * @author Sijay
...
public class SysNoticeServiceImpl extends ServiceImpl<SysNoticeMapper, SysNotice> implements ISysNoticeService {
     ...
     */
    @Override
    public Page<SysNotice> page(SysNotice entity, PageQuery pageQuery) {
        return page(pageQuery.build(), buildQueryWrapper(entity, SysNotice.class));
    }
    ...
    public boolean update(SysNotice entity) {
        if (checkUnique(entity) && exists(entity.getId())) {
            SysNotice sysNotice = getById(entity.getId());
            BeanUtil.copyEntityProperties(entity, sysNotice);
    ...
    public String importData(MultipartFile file) {
        int successCount = 0;
        int failCount = 0;
        for (SysNotice entity : ExcelUtil.read(file, SysNotice.class)) {
            if (add(entity)) {
                successCount++;
            } else {
                failCount++;
