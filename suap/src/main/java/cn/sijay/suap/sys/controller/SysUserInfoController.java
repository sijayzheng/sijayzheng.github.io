...
/**
 * <p>
 * <em>SysUserInfoController 用户信息控制层</em>
 * </p>
 *
 * @author Sijay
...
public class SysUserInfoController extends BaseController {
    ...
    @GetMapping("getById")
    public Result<SysUserInfo> getById(@RequestParam("id") Long id) {
        return success(sysUserInfoService.getById(id));
    }
    ...
    @GetMapping("listAll")
    public Result<List<SysUserInfo>> listAll() {
        return success(sysUserInfoService.list());
    }

    /**
     * 分页查询
     *
     * @param sysUserInfo 查询条件，可以为空。如果为空，则查询所有记录。
    ...
    @ApiResponse(responseCode = "200", description = "用户信息信息列表，如果没有记录，则返回空列表。")
    @GetMapping("page")
    public PageResult<SysUserInfo> page(SysUserInfo sysUserInfo, PageQuery pageQuery) {
        return toPageResult(sysUserInfoService.page(sysUserInfo, pageQuery));
    }

    /**
     * 添加
     *
     * @param sysUserInfo 添加的用户信息信息，不能为空。如果为空，则添加失败。
     * @return 是否添加成功
     */
    ...
    @PostMapping("add")
    public Result<Boolean> add(@RequestBody SysUserInfo sysUserInfo) {
        return toBoolean(sysUserInfoService.add(sysUserInfo), OperateType.CREATE);
    }

    /**
     * 修改
     *
     * @param sysUserInfo 修改的用户信息信息，不能为空。如果为空，则添加失败。
     * @return 是否修改成功
     */
    ...
    @PostMapping("update")
    public Result<Boolean> update(@RequestBody SysUserInfo sysUserInfo) {
        return toBoolean(sysUserInfoService.update(sysUserInfo), OperateType.MODIFY);
    }

    /**
     * 删除
     *
     * @param ids 要删除的用户信息id，不能为空。如果为空，则删除失败。
     * @return 是否删除成功
     */
    @Log(value = "删除", operateType = OperateType.REMOVE)
    @Operation(summary = "删除")
    @Parameter(name = "ids", description = "要删除的用户信息id，不能为空。如果为空，则删除失败。")
    @ApiResponse(responseCode = "200", description = "是否删除成功")
    @PostMapping("remove")
    public Result<Boolean> remove(@RequestBody Ids<Long> ids) {
        return toBoolean(sysUserInfoService.remove(ids), OperateType.REMOVE);
    }
    ...
    public void export(HttpServletResponse response) {
        ExcelUtil.export(sysUserInfoService.list(), "用户信息", SysUserInfo.class, response);
    }

    /**
     * 下载用户信息_数据模板
     */
    @Log(value = "下载用户信息_数据模板", operateType = OperateType.DOWNLOAD)
    @Operation(summary = "下载用户信息_数据模板")
    @ApiResponse(responseCode = "200", description = "是否下载成功")
    @PostMapping("downloadTemplate")
    public void downloadTemplate(HttpServletResponse response) {
        ExcelUtil.exportTemplate("用户信息_数据模板", SysUserInfo.class, response);
    }
