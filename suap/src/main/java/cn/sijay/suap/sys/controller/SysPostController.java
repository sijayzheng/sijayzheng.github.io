...
/**
 * <p>
 * <em>SysPostController 岗位信息控制层</em>
 * </p>
 *
 * @author Sijay
 * @since 2024/02/04 15:17
 */
@Tag(name = "岗位信息", description = "/sysPost")
...
public class SysPostController extends BaseController {
    ...
    @ApiResponse(responseCode = "200", description = "岗位信息详细信息")
    @GetMapping("getById")
    public Result<SysPost> getById(@RequestParam("id") Long id) {
     ...
     * 查询所有记录
     *
     * @return 岗位信息信息列表，如果没有记录，则返回空列表。
     */
    @Log(value = "查询所有记录", operateType = OperateType.SELECT)
    @Operation(summary = "查询所有记录")
    @ApiResponse(responseCode = "200", description = "岗位信息信息列表，如果没有记录，则返回空列表。")
    @GetMapping("listAll")
    public Result<List<SysPost>> listAll() {
        return success(sysPostService.list());
    }

    /**
     * 分页查询
     *
     * @param sysPost   查询条件，可以为空。如果为空，则查询所有记录。
    ...
    @ApiResponse(responseCode = "200", description = "岗位信息信息列表，如果没有记录，则返回空列表。")
    @GetMapping("page")
    public PageResult<SysPost> page(SysPost sysPost, PageQuery pageQuery) {
        return toPageResult(sysPostService.page(sysPost, pageQuery));
    }

    /**
     * 添加
     *
     * @param sysPost 添加的岗位信息信息，不能为空。如果为空，则添加失败。
     * @return 是否添加成功
     */
    @Log(value = "添加", operateType = OperateType.CREATE)
    @Operation(summary = "添加")
    @Parameter(name = "sysPost", description = "添加的岗位信息信息，不能为空。如果为空，则添加失败。")
    @ApiResponse(responseCode = "200", description = "是否添加成功")
    @PostMapping("add")
    public Result<Boolean> add(@RequestBody SysPost sysPost) {
        return toBoolean(sysPostService.add(sysPost), OperateType.CREATE);
    }

    /**
     * 修改
     *
     * @param sysPost 修改的岗位信息信息，不能为空。如果为空，则添加失败。
     * @return 是否修改成功
     */
    @Log(value = "修改", operateType = OperateType.MODIFY)
    @Operation(summary = "修改")
    @Parameter(name = "sysPost", description = "修改的岗位信息信息，不能为空。如果为空，则添加失败。")
    @ApiResponse(responseCode = "200", description = "是否修改成功")
    @PostMapping("update")
    public Result<Boolean> update(@RequestBody SysPost sysPost) {
        return toBoolean(sysPostService.update(sysPost), OperateType.MODIFY);
    }

    /**
     * 删除
     *
     * @param ids 要删除的岗位信息id，不能为空。如果为空，则删除失败。
     * @return 是否删除成功
     */
    @Log(value = "删除", operateType = OperateType.REMOVE)
    @Operation(summary = "删除")
    @Parameter(name = "ids", description = "要删除的岗位信息id，不能为空。如果为空，则删除失败。")
    @ApiResponse(responseCode = "200", description = "是否删除成功")
    @PostMapping("remove")
    public Result<Boolean> remove(@RequestBody Ids<Long> ids) {
        return toBoolean(sysPostService.remove(ids), OperateType.REMOVE);
    }
    ...
    public Result<Void> importData(@RequestPart("file") MultipartFile file) {
        return success(sysPostService.importData(file));
    }
    ...
    public void export(HttpServletResponse response) {
        ExcelUtil.export(sysPostService.list(), "岗位信息", SysPost.class, response);
    }

    /**
     * 下载岗位信息_数据模板
     */
    @Log(value = "下载岗位信息_数据模板", operateType = OperateType.DOWNLOAD)
    @Operation(summary = "下载岗位信息_数据模板")
    @ApiResponse(responseCode = "200", description = "是否下载成功")
    @PostMapping("downloadTemplate")
    public void downloadTemplate(HttpServletResponse response) {
        ExcelUtil.exportTemplate("岗位信息_数据模板", SysPost.class, response);
    }
