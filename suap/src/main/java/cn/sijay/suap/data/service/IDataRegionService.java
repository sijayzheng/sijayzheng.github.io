...
/**
 * <p>
 * <em>IDataRegionService 行政区划数据服务层</em>
 * </p>
 *
 * @author Sijay
 * @since 2024/02/04 15:17
 */
public interface IDataRegionService extends IService<DataRegion>, IBaseService<DataRegion> {
     ...
     */
    default boolean checkUnique(DataRegion entity) {
        if (StringUtil.isNotBlank(entity.getCode())) {
            QueryWrapper<DataRegion> wrapper = new QueryWrapper<>();
    ...
    default boolean exists(Long id) {
        return exists(new LambdaQueryWrapper<DataRegion>().eq(DataRegion::getId, id));
    }
     ...
     * @return 查询条件构造器
     */
    default LambdaQueryWrapper<DataRegion> buildWrapper(DataRegion entity) {
        LambdaQueryWrapper<DataRegion> wrapper = new LambdaQueryWrapper<>();
        wrapper.like(StringUtil.isNotBlank(entity.getName()), DataRegion::getName, entity.getName());
        wrapper.like(StringUtil.isNotBlank(entity.getCode()), DataRegion::getCode, entity.getCode());
        return wrapper;
    }
