  ...
  <div class="p-2">
    ...
    <el-card shadow="hover">
      ...
      <el-table v-loading="loading" :data="dataList" @selection-change="selectionChange">
        ...
        <el-table-column :show-overflow-tooltip="true" label="上级" prop="parentId"/>
        ...
        <el-table-column :show-overflow-tooltip="true" label="级别" prop="level"/>
          ...
          >
            <el-form-item label="上级：" prop="parentId">
              <el-select v-model="form.parentId" placeholder="请选择上级" clearable>
                <el-option v-for="item in parentIdOptions" :key="item.value" :label="item.label" :value="item.value"/>
              </el-select>
            </el-form-item>
            <el-form-item label="名称：" prop="name">
            ...
            <el-form-item label="级别：" prop="level">
              <el-select v-model="form.level" placeholder="请选择级别" clearable>
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.title" align-center>
      <el-button icon="Download" plain type="primary" @click="downloadTemplate()">下载模板</el-button>
      <el-upload :action="`${constant.baseApi}/dataRegion/import`" accept=".xls,.xlsx" :on-success="uploadSuccess" drag>
...
const rules = ref({
  ...
  name: [relus.required('名称'), relus.maxLength('名称', 50),],
  code: [relus.required('编码'), relus.maxLength('编码', 50),],
  level: [relus.required('级别'),],
})
...
const pageList = async () => {
  loading.value = true;
  dataRegionApi.page(queryParams.value).then(res => {
...
const add = () => {
  reset()
  drawer.visible = true;
  drawer.title = "添加行政区划数据";
}
/** 修改 */
const update = async (row) => {
  reset()
  const id = row?.id;
  dataRegionApi.getById(id).then(res => {
    Object.assign(form.value, res.data)
    drawer.visible = true;
    drawer.title = "修改行政区划数据";
  })
}
/** 提交按钮 */
const submitForm = () => {
  formRef.value?.validate((valid) => {
    if (valid) {
      if (form.value.id) {
        dataRegionApi.update(form.value).then(res => {
          showMsg(res)
        })
      } else {
        dataRegionApi.add(form.value).then(res => {
          showMsg(res)
        })
      }
    }
...
const remove = (row) => {
  ...
  ElMessageBox.confirm('是否确认删除编号为"' + _ids + '"的行政区划数据数据项？').then(() => {
    dataRegionApi.remove({
      id: row?.id,
      ids: ids.value
    }).then(res => {
      if (res.data) {
...
const downloadTemplate = () => {
  dataRegionApi.downloadTemplate()
}
/** 导出 */
const exportData = () => {
  dataRegionApi.exportData()
}
/** 导入结果提示 */
...
onMounted(() => {
  pageList()
  commonApi.listTableDataOptions('data_region').then(res => parentIdOptions.value = res.data)
  commonApi.listDictDataOptions('region_level').then(res => levelOptions.value = res.data)
})
</script>
