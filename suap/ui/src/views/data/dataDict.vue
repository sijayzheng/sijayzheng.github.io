  ...
  <div class="p-2">
      ...
      <el-card shadow="hover">
        <el-form ref="queryFormRef" :inline="true" :model="queryParams" class="inline-form">
          <el-form-item label="字典名称：" prop="name">
            <el-input v-model="queryParams.name" clearable placeholder="请输入字典名称" @keyup.enter="query"/>
          </el-form-item>
          <el-form-item label="字典编码：" prop="code">
            <el-input v-model="queryParams.code" clearable placeholder="请输入字典编码" @keyup.enter="query"/>
          </el-form-item>
          <el-form-item>
    ...
    <el-card shadow="hover">
      ...
      <el-table v-loading="loading" :data="dataList" @selection-change="selectionChange">
        ...
        <el-table-column :show-overflow-tooltip="true" label="字典名称" prop="name"/>
        <el-table-column :show-overflow-tooltip="true" label="字典编码" prop="code"/>
        <el-table-column :show-overflow-tooltip="true" label="字典值" prop="value"/>
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.title" align-center>
      <el-button icon="Download" plain type="primary" @click="downloadTemplate()">下载模板</el-button>
      <el-upload :action="`${constant.baseApi}/dataDict/import`" accept=".xls,.xlsx" :on-success="uploadSuccess" drag>
...
const rules = ref({
  ...
  name: [relus.required('字典名称'), relus.maxLength('字典名称', 50),],
  code: [relus.required('字典编码'), relus.maxLength('字典编码', 50),],
  value: [relus.maxLength('字典值', 50),],
})
...
const pageList = async () => {
  loading.value = true;
  dataDictApi.page(queryParams.value).then(res => {
...
const update = async (row) => {
  reset()
  const id = row?.id;
  dataDictApi.getById(id).then(res => {
...
const submitForm = () => {
  formRef.value?.validate((valid) => {
    if (valid) {
      if (form.value.id) {
        dataDictApi.update(form.value).then(res => {
          showMsg(res)
        })
      } else {
        dataDictApi.add(form.value).then(res => {
          showMsg(res)
        })
      }
    }
...
const remove = (row) => {
  ...
  ElMessageBox.confirm('是否确认删除编号为"' + _ids + '"的数据字典数据项？').then(() => {
    dataDictApi.remove({
      id: row?.id,
      ids: ids.value
    }).then(res => {
...
const downloadTemplate = () => {
  dataDictApi.downloadTemplate()
}
/** 导出 */
const exportData = () => {
  dataDictApi.exportData()
}
/** 导入结果提示 */
...
onMounted(() => {
  pageList()
  commonApi.listTableDataOptions('data_dict').then(res => parentIdOptions.value = res.data)
})
</script>
