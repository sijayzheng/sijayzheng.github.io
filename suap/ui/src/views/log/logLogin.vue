<template>
  <div class="p-2">
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.title" align-center>
      <el-button icon="Download" plain type="primary" @click="downloadTemplate()">下载模板</el-button>
      <el-upload :action="`${constant.baseApi}/logLogin/import`" accept=".xls,.xlsx" :on-success="uploadSuccess" drag>
...
const pageList = async () => {
  loading.value = true;
  logLoginApi.page(queryParams.value).then(res => {
...
const submitForm = () => {
  formRef.value?.validate((valid) => {
    if (valid) {
      if (form.value.id) {
        logLoginApi.update(form.value).then(res => {
          showMsg(res)
        })
      } else {
        logLoginApi.add(form.value).then(res => {
          showMsg(res)
        })
      }
    }
...
const remove = (row) => {
  ...
  ElMessageBox.confirm('是否确认删除编号为"' + _ids + '"的登录日志数据项？').then(() => {
    logLoginApi.remove({
      id: row?.id,
      ids: ids.value
    }).then(res => {
