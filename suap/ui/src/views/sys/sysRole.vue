<template>
  <div>
      ...
      <el-card shadow="hover">
        <el-form ref="queryFormRef" :inline="true" :model="queryParams" class="inline-form">
          <el-form-item label="角色名称：" prop="name">
            <el-input v-model="queryParams.name" clearable placeholder="请输入角色名称" @keyup.enter="query"/>
          </el-form-item>
          <el-form-item label="角色编码：" prop="code">
            <el-input v-model="queryParams.code" clearable placeholder="请输入角色编码" @keyup.enter="query"/>
          </el-form-item>
    ...
    <el-card shadow="hover">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button v-hasPermission="'sys:sysRole:add'" icon="Plus" plain type="primary" @click="add">新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button v-hasPermission="'sys:sysRole:remove'" :disabled="!ids.length" icon="Delete" plain type="danger" @click="remove">删除
            </el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button v-hasPermission="'sys:sysRole:import'" icon="Upload" plain type="primary" @click="uploadDialog.visible=true">导入</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button v-hasPermission="'sys:sysRole:export'" icon="Download" plain type="primary" @click="exportData">导出</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button v-hasPermission="'sys:sysRole:export'" icon="Download" plain type="primary" @click="downloadTemplate">下载模板</el-button>
          </el-col>
      ...
      <el-table v-loading="loading" :data="dataList" stripe @selection-change="selectionChange">
        ...
        <el-table-column :show-overflow-tooltip="true" label="角色名称" prop="name"/>
        <el-table-column :show-overflow-tooltip="true" label="角色编码" prop="code"/>
        ...
        <el-table-column align="center" class-name="small-padding fixed-width" label="操作" width="150">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button v-hasPermission="'sys:sysRole:edit'" icon="Edit" link type="primary" @click="edit(scope.row)"></el-button>
            </el-tooltip>
            <el-tooltip content="删除" placement="top">
              <el-button v-hasPermission="'sys:sysRole:remove'" icon="Delete" link type="primary" @click="remove(scope.row)"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.title">
      <el-scrollbar>
        <el-form ref="formRef" :model="formData" :rules="rules" label-position="left" label-width="120px" status-icon>
          ...
          <el-form-item label="角色编码：" prop="code">
            <el-input v-model="formData.code" clearable placeholder="请输入角色编码"/>
          </el-form-item>
    ...
    <el-dialog v-model="uploadDialog.visible" :title="uploadDialog.title" align-center>
      ...
      <el-upload :action="`${constant.baseApi}/sysRole/import`" :on-success="uploadSuccess" accept=".xlsx" drag>
...
const rules = ref({
  name: [relus.required('角色名称'), relus.maxLength('角色名称', 50),],
  code: [relus.required('角色编码'), relus.maxLength('角色编码', 50),],
})
...
const pageList = async () => {
  loading.value = true
  sysRoleApi.page(queryParams.value).then(res => {
...
const submitForm = () => {
  formRef.value?.validate((valid) => {
    if (valid) {
      sysRoleApi.save(formData.value).then(res => {
...
const remove = (row) => {
  ...
  ElMessageBox.confirm('是否确认删除编号为"' + _ids + '"的角色数据项？').then(() => {
    sysRoleApi.remove(_ids).then(res => {
      ElMessage.success(res.message)
      pageList()
    })
  }).catch((err) => {
