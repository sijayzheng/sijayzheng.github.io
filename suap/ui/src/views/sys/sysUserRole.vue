<template>
  <div class="p-2">
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.title" align-center>
      <el-button icon="Download" plain type="primary" @click="downloadTemplate()">下载模板</el-button>
      <el-upload :action="`${constant.baseApi}/sysUserRole/import`" accept=".xls,.xlsx" :on-success="uploadSuccess" drag>
...
const pageList = async () => {
  loading.value = true;
  sysUserRoleApi.page(queryParams.value).then(res => {
...
const update = async (row) => {
  reset()
  const id = row?.id;
  sysUserRoleApi.getById(id).then(res => {
...
const submitForm = () => {
  formRef.value?.validate((valid) => {
    if (valid) {
      if (form.value.id) {
        sysUserRoleApi.update(form.value).then(res => {
          showMsg(res)
        })
      } else {
        sysUserRoleApi.add(form.value).then(res => {
          showMsg(res)
        })
      }
    }
...
const remove = (row) => {
  ...
  ElMessageBox.confirm('是否确认删除编号为"' + _ids + '"的用户角色数据项？').then(() => {
    sysUserRoleApi.remove({
      id: row?.id,
      ids: ids.value
    }).then(res => {
      if (res.data) {
...
const downloadTemplate = () => {
  sysUserRoleApi.downloadTemplate()
}
/** 导出 */
const exportData = () => {
  sysUserRoleApi.exportData()
}
/** 导入结果提示 */
