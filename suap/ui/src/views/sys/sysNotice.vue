<template>
  <div class="p-2">
      ...
      <el-card shadow="hover">
        <el-form ref="queryFormRef" :inline="true" :model="queryParams" class="inline-form">
          ...
          <el-form-item label="内容：" prop="content">
            <el-input v-model="queryParams.content" clearable placeholder="请输入内容" @keyup.enter="query"/>
          </el-form-item>
          <el-form-item>
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.title" align-center>
      <el-button icon="Download" plain type="primary" @click="downloadTemplate()">下载模板</el-button>
      <el-upload :action="`${constant.baseApi}/sysNotice/import`" accept=".xls,.xlsx" :on-success="uploadSuccess" drag>
...
const rules = ref({
  ...
  content: [relus.required('内容'), relus.maxLength('内容', 500),],
  type: [relus.required('类型'), relus.maxLength('类型', 10),],
  status: [relus.required('状态'), relus.maxLength('状态', 10),],
})
...
const submitForm = () => {
  formRef.value?.validate((valid) => {
    if (valid) {
      if (form.value.id) {
        sysNoticeApi.update(form.value).then(res => {
          showMsg(res)
        })
      } else {
        sysNoticeApi.add(form.value).then(res => {
          showMsg(res)
        })
      }
    }
...
onMounted(() => {
  pageList()
  commonApi.listDictDataOptions('sys_notice_type').then(res => typeOptions.value = res.data)
  commonApi.listDictDataOptions('sys_notice_status').then(res => statusOptions.value = res.data)
})
</script>
