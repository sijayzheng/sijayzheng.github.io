<template>
  <div class="p-2">
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.title" align-center>
      <el-button icon="Download" plain type="primary" @click="downloadTemplate()">下载模板</el-button>
      <el-upload :action="`${constant.baseApi}/sysRoleMenu/import`" accept=".xls,.xlsx" :on-success="uploadSuccess" drag>
...
const pageList = async () => {
  loading.value = true;
  sysRoleMenuApi.page(queryParams.value).then(res => {
...
const update = async (row) => {
  reset()
  const id = row?.id;
  sysRoleMenuApi.getById(id).then(res => {
...
const submitForm = () => {
  formRef.value?.validate((valid) => {
    if (valid) {
      if (form.value.id) {
        sysRoleMenuApi.update(form.value).then(res => {
          showMsg(res)
        })
      } else {
        sysRoleMenuApi.add(form.value).then(res => {
          showMsg(res)
        })
      }
    }
...
const remove = (row) => {
  ...
  ElMessageBox.confirm('是否确认删除编号为"' + _ids + '"的角色菜单数据项？').then(() => {
    sysRoleMenuApi.remove({
      id: row?.id,
      ids: ids.value
    }).then(res => {
      if (res.data) {
...
const downloadTemplate = () => {
  sysRoleMenuApi.downloadTemplate()
}
/** 导出 */
const exportData = () => {
  sysRoleMenuApi.exportData()
}
/** 导入结果提示 */
