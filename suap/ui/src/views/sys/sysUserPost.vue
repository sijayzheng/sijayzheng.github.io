<template>
  <div class="p-2">
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.title" align-center>
      <el-button icon="Download" plain type="primary" @click="downloadTemplate()">下载模板</el-button>
      <el-upload :action="`${constant.baseApi}/sysUserPost/import`" accept=".xls,.xlsx" :on-success="uploadSuccess" drag>
...
const pageList = async () => {
  loading.value = true;
  sysUserPostApi.page(queryParams.value).then(res => {
...
const update = async (row) => {
  reset()
  const id = row?.id;
  sysUserPostApi.getById(id).then(res => {
...
const submitForm = () => {
  formRef.value?.validate((valid) => {
    if (valid) {
      if (form.value.id) {
        sysUserPostApi.update(form.value).then(res => {
          showMsg(res)
        })
      } else {
        sysUserPostApi.add(form.value).then(res => {
          showMsg(res)
        })
      }
    }
...
const remove = (row) => {
  ...
  ElMessageBox.confirm('是否确认删除编号为"' + _ids + '"的用户岗位数据项？').then(() => {
    sysUserPostApi.remove({
      id: row?.id,
      ids: ids.value
    }).then(res => {
      if (res.data) {
...
const downloadTemplate = () => {
  sysUserPostApi.downloadTemplate()
}
/** 导出 */
const exportData = () => {
  sysUserPostApi.exportData()
}
/** 导入结果提示 */
