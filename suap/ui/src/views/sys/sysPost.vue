  ...
  <div class="p-2">
    ...
    <el-card shadow="hover">
      ...
      <el-table v-loading="loading" :data="dataList" @selection-change="selectionChange">
        ...
        <el-table-column :show-overflow-tooltip="true" label="岗位名称" prop="name"/>
        <el-table-column :show-overflow-tooltip="true" label="岗位编码" prop="code"/>
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.title" align-center>
      <el-button icon="Download" plain type="primary" @click="downloadTemplate()">下载模板</el-button>
      <el-upload :action="`${constant.baseApi}/sysPost/import`" accept=".xls,.xlsx" :on-success="uploadSuccess" drag>
...
const rules = ref({
  name: [relus.required('岗位名称'), relus.maxLength('岗位名称', 50),],
  code: [relus.required('岗位编码'), relus.maxLength('岗位编码', 50),],
})
...
const pageList = async () => {
  loading.value = true;
  sysPostApi.page(queryParams.value).then(res => {
...
const update = async (row) => {
  reset()
  const id = row?.id;
  sysPostApi.getById(id).then(res => {
...
const submitForm = () => {
  formRef.value?.validate((valid) => {
    if (valid) {
      if (form.value.id) {
        sysPostApi.update(form.value).then(res => {
          showMsg(res)
        })
      } else {
        sysPostApi.add(form.value).then(res => {
          showMsg(res)
        })
      }
    }
...
const remove = (row) => {
  ...
  ElMessageBox.confirm('是否确认删除编号为"' + _ids + '"的岗位信息数据项？').then(() => {
    sysPostApi.remove({
      id: row?.id,
      ids: ids.value
    }).then(res => {
...
const downloadTemplate = () => {
  sysPostApi.downloadTemplate()
}
/** 导出 */
const exportData = () => {
  sysPostApi.exportData()
}
/** 导入结果提示 */
