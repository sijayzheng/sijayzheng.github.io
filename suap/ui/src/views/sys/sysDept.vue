<template>
  <div class="p-2">
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.title" align-center>
      <el-button icon="Download" plain type="primary" @click="downloadTemplate()">下载模板</el-button>
      <el-upload :action="`${constant.baseApi}/sysDept/import`" accept=".xls,.xlsx" :on-success="uploadSuccess" drag>
...
const pageList = async () => {
  loading.value = true;
  sysDeptApi.page(queryParams.value).then(res => {
...
const submitForm = () => {
  formRef.value?.validate((valid) => {
    if (valid) {
      if (form.value.id) {
        sysDeptApi.update(form.value).then(res => {
          showMsg(res)
        })
      } else {
        sysDeptApi.add(form.value).then(res => {
          showMsg(res)
        })
      }
    }
...
const remove = (row) => {
  ...
  ElMessageBox.confirm('是否确认删除编号为"' + _ids + '"的部门信息数据项？').then(() => {
    sysDeptApi.remove({
      id: row?.id,
      ids: ids.value
    }).then(res => {
...
const downloadTemplate = () => {
  sysDeptApi.downloadTemplate()
}
/** 导出 */
const exportData = () => {
  sysDeptApi.exportData()
}
/** 导入结果提示 */
...
onMounted(() => {
  pageList()
  commonApi.listTableDataOptions('sys_dept').then(res => parentIdOptions.value = res.data)
  commonApi.listTableDataOptions('sys_user').then(res => leaderOptions.value = res.data)
})
</script>
