...
function addGZTMenuIndex(json,mainmenuid){
  	if(json){  
  		var isIndex=false;//链接页面是否显示菜单
        var parentid="";
        var parenturl="";
		...
		for(i in json){
			...
			if(isIndex){
				for(j in json[i] ){
					if((json[i][j].parent_id==parentid)&&(json[i][j].id!=json[i][j].parent_id)){
						mainMenuIndex=mainMenuIndex.concat(json[i][j]);//取得显示页面中的菜单数值
					}  
				}
			}   
		...
		mainMenuIndex.sort(AscSort);
		returnStr=addGZTMainItemIndex(mainMenuIndex);
	 }else{
       returnStr="";
     }
     return returnStr;
...
function addMainItemwindow(list){
    ...
    for(var i=0;i<list.length;i++){
    	if((list[i].displayindex+"").substr(0,1)=="1"){
    		html1+=" <a href='javascript:void(0);' rel='"+list[i].url+"'>"+list[i].names+"</a>";
    	}else if((list[i].displayindex+"").substr(0,1)=="2"){
    		html2+="  <a href='javascript:void(0);' rel='"+list[i].url+"'>"+list[i].names+"</a>";
    	}else if((list[i].displayindex+"").substr(0,1)=="3"){
    		html3+="  <a href='javascript:void(0);' rel='"+list[i].url+"'>"+list[i].names+"</a>";
    	}else if((list[i].displayindex+"").substr(0,1)=="4"){
    		html4+="  <a href='javascript:void(0);' rel='"+list[i].url+"'>"+list[i].names+"</a>";
    	}else if((list[i].displayindex+"").substr(0,1)=="5"){
    		html5+="  <a href='javascript:void(0);' rel='"+list[i].url+"'>"+list[i].names+"</a>";
    	}
 	}
...
{        
    var returnStr="";
        ...
        for(var i=0;i<list.length;i++){
		    if(i==0){
		    	returnStr+="<li><a href=\"javascript:changeSrc('"+list[i].url+"')\" id=\"one\" target=\"_parent\" class=\"selected\"><img src=\"/ITMIS/images/loginimages/i07.png\" title=\""+list[i].names+"\" height=\"40\"/><h2>"+list[i].names+"</h2></a></li>";
		    }else{
		    	returnStr+="<li><a href=\"javascript:changeSrc('"+list[i].url+"')\" id=\"one\" target=\"_parent\"><img src=\"/ITMIS/images/loginimages/icon0"+(i+1)+".png\" title=\""+list[i].names+"\" height=\"40\"/><h2>"+list[i].names+"</h2></a></li>";
		    }  
		}
		returnStr+="</ul>";
		return returnStr; 
...
function addMainItemIndex(list){
    ...
    var html1="";
    for(var i=0;i<list.length;i++){
    		var views = list[i].views;
    		if(views != ""){
    			views = views.split("/")[2];
    		}
    		if(i==0){
        		html1+="<a href=\"javascript:void(0);\" rel=\""+list[i].url+"\" class=\"la"+i+" cur\"><i class='iconfont icon-"+views+"' style = 'font-size:25px;color:white;position:absolute;top:18px;left:8px;opacity:0.5;font-weight:normal'></i><b>"+list[i].names+"</b></a>";
    		}else{
        		html1+="<a href=\"javascript:void(0);\" rel=\""+list[i].url+"\" class=\"la"+i+"\"><i class='iconfont icon-"+views+"' style = 'font-size:25px;color:white;position:absolute;top:18px;left:8px;opacity:0.5;font-weight:normal'></i><b>"+list[i].names+"</b></a>";
    		}
...
function addGZTMainItemIndex(list){
	list.sort(AscSortMenu);
	list.sort(AscSort);
    var returnStr="";
    var html1="";
 	for(var i=0;i<list.length;i++){
		    ...
		    if(i==0){
        		html1+="<a href=\"javascript:changeSrc('"+list[i].names+"','"+list[i].url+"');\"  class=\"la"+i+" cur\"><span><b>"+list[i].names+"</b></span></a>";
    		}else{
        		html1+="<a href=\"javascript:changeSrc('"+list[i].names+"','"+list[i].url+"');\"  class=\"la"+i+"\"><span><b>"+list[i].names+"</b></span></a>";
    		}
 	
    		/*if(i==0){
        		html1+="<a href=\"javascript:void(0);\" rel=\""+list[i].url+"\" class=\"la"+i+" cur\"><span><b>"+list[i].names+"</span></b></a>";
    		}else{
        		html1+="<a href=\"javascript:void(0);\" rel=\""+list[i].url+"\" class=\"la"+i+"\"><span><b>"+list[i].names+"</span></b></a>";
    		}*/
 	}
 	returnStr=html1;
 	//alert(returnStr);
