...
function pickerDateRange(inputId, options) {
  ...
  var wrapper = {
    gri: [
      ...
      '<input type="text" class="gri_dateRangeInput" name="' +
        this.startDateId +
        '" id="' +
        this.startDateId +
        '" value="' +
        this.mOpts.startDate +
        '"  />',
      ...
      '<input type="text" class="gri_dateRangeInput" name="' +
        this.startCompareDateId +
        '" id="' +
        this.startCompareDateId +
        '" value="' +
        this.mOpts.startCompareDate +
        '"  />',
      '<span class="' + this.mOpts.joinLineId + '"> - </span>',
      '<input type="text" class="gri_dateRangeInput" name="' +
        this.endCompareDateId +
        '" id="' +
        this.endCompareDateId +
        '" value="' +
        this.mOpts.endCompareDate +
        '"  />',
      '</div>',
      '<div>',
    ...
    ta: [
      ...
      '<input type="text" class="ta_ipt_text_s" name="' +
        this.startDateId +
        '" id="' +
        this.startDateId +
        '" value="' +
      ...
      '</div>',
      '<div class="frm_btn">',
      '<input class="ta_btn ta_btn_primary" type="button" name="' +
        this.submitBtn +
        '" id="' +
        this.submitBtn +
        '" value="确定" />',
  ...
  var checkBoxWrapper = {
    gri: [
      ...
      '<input type="checkbox" class="gri_pc" name="' +
        this.compareCheckboxId +
        '" id="' +
        this.compareCheckboxId +
        '" value="1"/>对比',
      '</label>',
    ...
    ta: [
      ...
      '<input type="checkbox" class="pc" name="' +
        this.compareCheckboxId +
        '" id="' +
        this.compareCheckboxId +
        '" value="1"/>对比',
  ...
  if (1 > $('#' + this.mOpts.startDateId).length) {
    $('' != this.mOpts.target ? '#' + this.mOpts.target : 'body').append(
      '<input type="hidden" id="' +
        this.mOpts.startDateId +
        '" name="' +
        this.mOpts.startDateId +
        '" value="' +
        this.mOpts.startDate +
  ...
  $('#' + this.compareCheckboxId).bind('click', function () {
    ...
    if ($('#' + __method.compareCheckboxId).attr('checked')) {
      ...
      if ('' == scDate || '' == ecDate) {
        ...
        if (
          ecDate.getTime() >= __method.mOpts.minValidDate * 1000 &&
          scDate.getTime() < __method.mOpts.minValidDate * 1000
        ) {
...
pickerDateRange.prototype.init = function (isCompare) {
  ...
  var minDate, maxDate
  var isNeedCompare =
    typeof isCompare != 'undefined'
      ? isCompare && $('#' + __method.compareCheckboxId).attr('checked')
      : $('#' + __method.compareCheckboxId).attr('checked')
  // 清空日期列表的内容
  ...
  if (this.mOpts.magicSelect && this.mOpts.theme == 'ta') {
    ...
    do {
      ...
      if (i == 0) {
        td = this.fillDate(
          this.str2date($('#' + this.endDateId).val()).getFullYear(),
          this.str2date($('#' + this.endDateId).val()).getMonth(),
          i
        )
        $('#' + this.dateListId).append(td)
      } else {
        td = this.fillDate(
          this.str2date($('#' + this.startDateId).val()).getFullYear(),
          this.str2date($('#' + this.startDateId).val()).getMonth(),
          i
        )
        var firstTd =
          this.mOpts.theme == 'ta'
    ...
    // 日历开始时间
    this.calendar_startDate = new Date(
      this.str2date($('#' + this.startDateId).val()).getFullYear(),
      this.str2date($('#' + this.startDateId).val()).getMonth(),
      1
    )
  ...
  for (var property in __method.periodObj) {
    if ($('#' + property).length > 0) {
      ...
      $('#' + property).bind('click', function () {
        ...
        __method.mOpts.theme == 'ta'
          ? $('#' + __method.compareInputDiv).hide()
            ...
            .each(function () {
              $(this).click(function () {
                ...
                if (idx ^ (1 == 0)) {
                  ...
                  nt = __method.fillDate(
                    __method.calendar_startDate.getFullYear(),
                    __method.calendar_startDate.getMonth(),
                    idx
                  )
                } else {
                  //结束
                  ...
                  __method.mOpts.endDate = __method.date2ymd(__method.calendar_endDate).join('-')
                  nt = __method.fillDate(
                    __method.calendar_endDate.getFullYear(),
                    __method.calendar_endDate.getMonth(),
                    idx
                  )
                }
                var tb = $('#' + __method.dateListId)
                  .find('table')
                  .get(idx ^ 1)
                $(tb).replaceWith(nt)
                //}
...
pickerDateRange.prototype.getSpecialPeriod = function (period) {
  ...
  ;(true == __method.mOpts.isTodayValid && '' != __method.mOpts.isTodayValid) || 2 > period
    ? ''
    : date.setTime(date.getTime() - 1 * 24 * 60 * 60 * 1000)
  var timeStamp =
    date.getTime() - period * 24 * 60 * 60 * 1000 < __method.mOpts.minValidDate * 1000
      ? __method.mOpts.minValidDate * 1000
      : date.getTime() - period * 24 * 60 * 60 * 1000
...
pickerDateRange.prototype.removeCSS = function (isCompare, specialClass) {
  ...
  for (var d = new Date(bDate); d.getTime() <= e_date.getTime(); d.setDate(d.getDate() + 1)) {
    ...
    $('#' + this.calendarId + '_' + this.date2ymd(d).join('-'))
      .removeClass(this.mOpts.firstCss)
      .removeClass(this.mOpts.lastCss)
...
pickerDateRange.prototype.addCSS = function (isCompare, specialClass) {
  ...
  if (this.mOpts.theme == 'ta') {
    //为开始结束添加特殊样式
    $('#' + this.calendarId + '_' + this.date2ymd(new Date(sDate)).join('-'))
      .removeClass()
      .addClass(this.mOpts.firstCss)
    $('#' + this.calendarId + '_' + this.date2ymd(new Date(eDate)).join('-'))
      .removeClass()
      .addClass(this.mOpts.lastCss)
    //如果开始结束时间相同
...
pickerDateRange.prototype.checkDateRange = function (startYmd, endYmd) {
  ...
  if (eTime >= sTime) {
    // 判断是否超过最大日期外
    if (this.mOpts.monthRangeMax == '1') {
      maxEDate = this.str2date(startYmd)
      minEDate = this.str2date(endYmd)
      if (maxEDate.getMonth() != minEDate.getMonth()) {
        alert('日期选择不能跨月！')
        return false
      }
    } else {
...
pickerDateRange.prototype.selectDate = function (ymd) {
  ...
  if (this.startDateId == this.dateInput) {
    ...
    //alert('bbb');
    // 为当前点加样式
    $('#' + this.calendarId + '_' + ymd).attr(
      'class',
      this.mOpts.theme == 'ta' ? this.mOpts.clickCss : this.mOpts.theme + '_' + this.mOpts.selectCss
    )
    // 获取开始时间的初始值
    ...
    $('#' + this.calendarId + '_' + ymd).attr(
      'class',
      this.mOpts.theme == 'ta' ? this.mOpts.clickCss : this.mOpts.theme + '_' + this.mOpts.compareCss
    )
    // 获取开始时间的初始值
    this.startDefDate = $('#' + this.dateInput).val()
    // 更改对应输入框的值
...
pickerDateRange.prototype.show = function (isCompare, __method) {
  ...
  if (0 < clientWidth && $('#' + this.calendarId).width() + pos.left > clientWidth) {
    left =
      pos.left +
      $('#' + this.inputId).width() -
      $('#' + this.calendarId).width() +
      (/msie/i.test(navigator.userAgent) && !/opera/i.test(navigator.userAgent) ? 5 : 0)
...
pickerDateRange.prototype.close = function (btnSubmit) {
  ...
  if (btnSubmit) {
    ...
    var nDateTime =
      true == this.mOpts.isTodayValid && '' != this.mOpts.isTodayValid
        ? new Date().getTime()
    ...
    if (this.mOpts.theme != 'ta') {
      if ('' != $('#' + this.startCompareDateId).val() && '' != $('#' + this.endCompareDateId).val()) {
        ...
        if (_ecDateTime > nDateTime) {
          ...
          $('#' + this.startCompareDateId).val(
            this.formatDate(this.date2ymd(new Date(_ecDateTime + bDateTime - eDateTime)).join('-'))
          )
        }
    ...
    if (this.mOpts.monthRangeMax == '1') {
      maxEDate = this.str2date($('#' + this.startDateId).val())
      minEDate = this.str2date($('#' + this.endDateId).val())
      //alert(maxEDate+"**"+minEDate);
      if (maxEDate.getMonth() != minEDate.getMonth()) {
        alert('日期选择不能跨月！')
        return false
      }
      ...
      this.mOpts.shortOpr == true
        ...
        : $('#' + this.startCompareDateId).val() +
          ('' == $('#' + this.endCompareDateId).val()
            ? ''
            : this.mOpts.defaultText + $('#' + this.endCompareDateId).val())
    if (input && input.tagName == 'INPUT') {
...
pickerDateRange.prototype.fillDate = function (year, month, index) {
  ...
  if (isTaTheme) {
    ...
    if (this.mOpts.magicSelect) {
      var yh = [
        '<div class="select_wrap" id="yselected_' +
          index +
          '"><div class="select" id="_ybtn_' +
          index +
          '">' +
          year +
          '</div><div class="dropdown" id="_yctn_' +
          index +
          '"><ul class="list_menu">',
      ]
      var mh = [
        '<div class="select_wrap" id="mselected_' +
          index +
          '"><div class="select" id="_mbtn_' +
          index +
          '">' +
          (month + 1) +
          '</div><div class="dropdown" id="_mctn_' +
          index +
          '"><ul class="list_menu">',
      ]
      ...
      mh.push('</ul></div></div>')
      $(cap).append(
        yh.join('') + '<span class="joinLine"> 年 </span>' + mh.join('') + '<span class="joinLine"> 月 </span>'
      )
    } else {
    ...
    if (0 == index) {
      $(td).append(
        '<a href="javascript:void(0);" id="' + this.nextMonth + '" class="gri_dateRangeNextMonth"><span>next</span></a>'
      )
    }
    // 如果是第一个月的日期，则加上上一个月的链接
    if (index + 1 == this.mOpts.calendars) {
      $(td).append(
        '<a href="javascript:void(0);" id="' + this.preMonth + '" class="gri_dateRangePreMonth"><span>pre</span></a>'
      )
    }
  ...
  for (var d = dateBegin; d.getTime() <= dateEnd.getTime(); d.setDate(d.getDate() + 1)) {
    if (d.getTime() < firstDayOfMonth.getTime()) {
      ...
      d.getTime() < __method.mOpts.minValidDate * 1000 ||
      ('' !== __method.mOpts.maxValidDate && d.getTime() > __method.mOpts.maxValidDate)
    ) {
      // 当前时间之后的日期，或者开启统计之前的日期
      if ((d.getDate() == dToday) & (this.mOpts.showtoday == true)) {
