    ...
    */
    ...
    function buildGotoAnchorHtml(component, gotoOptions, attrs, innerHtml) {
        ...
        if (!forceOff && component.opt('navLinks')) {
        	if(attrs.indexOf('fc-day-number')>0&&(new Date(gotoOptions)<new Date())){
        		innerHtml = ' <span style="color:red;float:left">⚐</span>'+innerHtml
        	}
            return '<a' + attrs +
                ' data-goto="' + htmlEscape(JSON.stringify(finalOptions)) + '" style="width:90%">' +
                innerHtml +
                '</a>';
        }
        else {
