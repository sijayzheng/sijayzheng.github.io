!function(window) {
  ...
  function getOffsetPosition(a) {
    ...
    if ('getBoundingClientRect' in document.documentElement) {
      var d = a.getBoundingClientRect(), e = a.ownerDocument, f = e.body, g = e.documentElement, h = g.clientTop || f.clientTop || 0,
        i = g.clientLeft || f.clientLeft || 0, b = d.top + (self.pageYOffset || g && g.scrollTop || f.scrollTop) - h,
  ...
  function b(c) {
    ...
    this.initialize = function() {
      ...
      var b = a.offsetLeft, c = a.offsetTop, f = this.mouseDownEvent.offsetLeft, g = this.mouseDownEvent.offsetTop, h = b >= f ? f : b,
        i = c >= g ? g : c, j = Math.abs(a.dx) * this.scaleX, k = Math.abs(a.dy) * this.scaleY, l = new d(h, i, j, k)
    ...
    this.paintCtrl = function(a) {
      ...
      if (null == this.selectedPoint) {
        ...
        if ('Top_Left' == this.selectedPoint) {
          var d = this.selectedSize.width - a.dx, e = this.selectedSize.height - a.dy, b = this.selectedLocation.x + a.dx,
  ...
  function f(b, c, g) {
    ...
    this.initialize = function(b, c, d) {
      ...
      var e = this.arrowsOffset, f = this.arrowsRadius / 2, g = c, h = d, i = Math.atan2(h.y - g.y, h.x - g.x),
        j = a.util.getDistance(g, h) - this.arrowsRadius, k = g.x + (j + e) * Math.cos(i), l = g.y + (j + e) * Math.sin(i), m = h.x + e * Math.cos(i),
        n = h.y + e * Math.sin(i)
      i -= Math.PI / 2
      var o = {
        x: k + f * Math.cos(i),
    ...
    return b.radius = 150, b.colors = ['#3666b0', '#2ca8e0', '#77d1f6'], b.datas = [.3, .3, .4], b.titles = ['A', 'B', 'C'], b.paint = function(a) {
      ...
      for (var e = 0, f = 0; f < this.datas.length; f++) {
        ...
        var h = this.titles[f] + ': ' + (100 * this.datas[f]).toFixed(2) + '%', i = a.measureText(h).width,
          j = (a.measureText('鐢�').width, (e + e + g) / 2), k = this.radius * Math.cos(j), l = this.radius * Math.sin(j)
  ...
  function l(b, c) {
    ...
    var f = c.p1, g = c.p2, h = (c.context, f.x + (g.x - f.x) / 2), i = f.y + (g.y - f.y) / 2, j = a.util.getDistance(f, g) / 2, k = Math.atan2(i, h),
